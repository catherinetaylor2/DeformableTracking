...
int SegmentImage(std::string PrevSeg, std::string filename){
    ...
    Mat img_grey, canny_edges;
    cvtColor(image, img_grey, CV_BGR2GRAY);
    img_grey = img_grey > 200;
   // blur( img_grey, img_grey, Size(3,3) );

    std::vector<std::vector<Point> > contours;
    std::vector<Vec4i> hierarchy;
  //  Canny( img_grey, canny_edges, contourThreshold, contourThreshold*3, 3 , true);
    findContours( img_grey, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE);
      
   Mat drawing2 = 255*Mat::ones( img_grey.size(), CV_8UC1 );
    for( int i = 0; i< contours.size(); i++ ){
      drawContours( drawing2, contours, i, Scalar(0,0,0), 3, 8, hierarchy, 0, Point() );
    }
    Mat canny_edges2;
    std::vector<std::vector<Point> > contours2;
    std::vector<Vec4i> hierarchy2;
    Canny( drawing2, canny_edges2, contourThreshold, contourThreshold*3, 3 , true);
    findContours( canny_edges2, contours2, hierarchy2, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE);
...
{ for( int i = 0; i < raw_dist.cols; i++ )
     { raw_dist.at<float>(j,i) = pointPolygonTest( contours2[0], Point2f(i,j), true ); 
    }
}
